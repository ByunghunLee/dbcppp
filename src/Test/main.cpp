...
//                                                                 Signal::ExtendedValueType::Integer);
    ...
    double dec_easy_object_triggered_sdm = easy_decode(*const_cast<Signal*>(OBJ_Triggered_SDM_0), data);
    double dec_sig_object_triggered_sdm = OBJ_Triggered_SDM_0->decode(&data[0]);

    BOOST_TEST(dec_easy_object_triggered_sdm == dec_sig_object_triggered_sdm);
    BOOST_TEST(OBJ_Triggered_SDM_0->rawToPhys(dec_sig_object_triggered_sdm) == 2);
    auto&& y = OBJ_Triggered_SDM_0->getValueDescription(2);
    if(y != nullptr)
    {
        std::cout << *y << std::endl;
    }
    ...
    double dec_easy_object_height_0 = easy_decode(*const_cast<Signal*>(OBJ_Height_0), data);
    double dec_sig_object_height_0 = OBJ_Height_0->decode(&data[0]);

    BOOST_TEST(dec_easy_object_height_0 == dec_sig_object_height_0);
    BOOST_TEST(OBJ_Height_0->rawToPhys(dec_sig_object_height_0) == 2);
    auto&& x =  OBJ_Height_0->getValueDescription(1);
    if(x != nullptr)
    {
        std::cout << *x << std::endl;
    }
