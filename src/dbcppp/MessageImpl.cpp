        ...
        , _comment(std::move(comment))
{
    for (auto& s : _signals)
    {
        _signals_ordered_by_start_bit.insert(std::make_pair(s.second.getStartBit(), std::move(static_cast<SignalImpl&>(*s.second.clone()))));
    }
}
...
void MessageImpl::forEachSignalOrderedByStartBit(std::function<void(const Signal&)>&& cb) const
{
    for (const auto& s : _signals_ordered_by_start_bit)
    {
        cb(s.second);
    }
}
